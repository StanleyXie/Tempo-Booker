...
class TempoTimeTracker {
  ...
  async initialize() {
    try {
      ...
      if (config.isOAuthMode) {
        this.logger.warn('📝 OAuth mode detected - make sure to complete OAuth flow');
      } else {
      ...
      console.log('3. Or manually edit config.yaml in your workspace directory');
      return false;
    }
  }
  ...
  static async silentImport(filePath, dateScope) {
    try {
      ...
      if (scope && scope !== 'all') {
        ...
        switch (scope) {
          ...
          case 'this-month':
            ...
            break;
          default:
            app.logger.warn(`Unknown date scope: ${scope}, importing all entries`);
        }
      ...
      await timeTrackingController.importWorklogs(importFile, dateFilter, app.logger);
...
if (require.main === module) {
  ...
  if (args.includes('--setup') || args.includes('-s')) {
    ...
    SetupWizard.checkAndRunSetup().then((setupRan) => {
      if (setupRan) {
...
async function processCommands(args) {
  if (args.length >= 2 && args[0] === 'quick') {
    ...
    console.log(chalk.yellow('  tempo-booker --setup                                # First-time setup wizard'));
    console.log(chalk.yellow('  node src/utils/get-account-id.js                    # Auto-retrieve your Account ID'));
    console.log(chalk.yellow('  node src/utils/find-account-id.js                   # Manual Account ID discovery'));
    console.log(chalk.yellow('  node src/utils/add-issue.js                         # Add issues to static mapping'));
    console.log(chalk.yellow('  Edit config.yaml to customize preferences'));
    console.log(chalk.white('\nSecurity & Token Management:'));
    console.log(chalk.cyan('  tempo-booker --security-status                      # Check token security status'));
    console.log(chalk.cyan('  tempo-booker --migrate-token                        # Migrate token to keychain'));
    console.log(chalk.cyan('  tempo-booker --delete-token                         # Remove stored token'));
    console.log(chalk.cyan('  tempo-booker --test-keychain                        # Test keychain functionality'));
    console.log(chalk.white('\nSilent Import Examples:'));
    console.log(chalk.gray('  tempo-booker import                                  # Use defaults from config.yaml'));
    console.log(chalk.gray('  tempo-booker import my-worklogs.csv                  # Import specific file'));
    console.log(chalk.gray('  tempo-booker import worklogs.csv current-week        # Import with date scope'));
    console.log(chalk.white('\nIssue Mapping (Manual):'));
    console.log(chalk.magenta('  tempo-booker manual-mapping <issue-key>             # Open browser for manual mapping'));
    console.log(chalk.magenta('  tempo-booker add-mapping <key> <id> "<summary>"     # Add issue mapping'));
    console.log(chalk.magenta('  tempo-booker list-mappings                          # Show current mappings'));
    console.log(chalk.magenta('  tempo-booker validate-mapping <issue-key>           # Validate existing mapping'));
    console.log(chalk.white('\nBeta Features (enable with function_beta: true):'));
    console.log(chalk.gray('  node bulk-map-issues.js [keys]                       # Bulk issue key mapping (deprecated)'));
    console.log(chalk.white('\nDate Scopes:'));
